...
1. **Install dependencies** in your clone:
...
5. **Pull down everything Peliqan currently has:**
   ...
   This creates `scripts/`, one `<id>_<slug>.py` file per Data App, plus
...
- **On-disk file**: what's in `scripts/<id>_<slug>.py` right now, *including*
  a local-dev shim (~20 lines) that fetch prepends so the script can also run
...
current version next to it as `<id>_<slug>.remote.py` instead.
...
- **Genuine conflict** (an `<id>_<slug>.remote.py` appears): open both files, decide
